...
import {View,Text,StyleSheet,ScrollView,TouchableOpacity,Image,Alert,ActivityIndicator, Modal,TextInput,StatusBar} from 'react-native';
      ...
      //console.log('token',token);
        const fetchInterestsData = async () => {
            if (!token) {
                ...
                GlobalToast.showError("Error", "No token found. Please login again.");
                return;
            }
            try {
...
//handleSaveInterests insert post method
  ...
  try {
    ...
    if (response.ok && json.success) {
      ...
      GlobalToast.showSuccess("Success", "Interests saved successfully.");
...
const fetchPersonalDetails = async () => {
  try {
    ...
    if (result.success) {
      ...
      lastFetchedData.current = result.data;
    } else {
      console.log('Failed to fetch personal details:', result.message);
    }
  } catch (error) {
...
const handleSaveAboutMe = async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      GlobalToast.showSuccess("Success", "About me updated successfully!");
      setAboutModalVisible(false);
    } else {
      //Alert.alert('Error', result.message || 'Update failed');
       GlobalToast.showError("Error", "Update failed");
    }
  } catch (error) {
    console.error('Error updating about_me:', error);
   // Alert.alert('Error', 'Something went wrong. Please try again.');
   GlobalToast.showError("Error", "Something went wrong. Please try again.");
  }
};

  // 👇 PLACE THE LOADER CHECK HERE
  if (loading) {
    return (
      <View style={{ flex:1, justifyContent:'center', alignItems:'center', backgroundColor:'#fff' }}>
        <ActivityIndicator size="large" color="#F93CA6" />
      </View>
    );
  }
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          ...
          <TouchableOpacity style={styles.headerIcon} onPress={() => navigation.navigate('Setting')}>
  ...
  style={[
    styles.progressRing,
    getProgressRingStyle(personalData?.match_parsentence || 0),
  ]}
>
<View style={styles.profileImagePlaceholder}>
  ...
  {/* Pencil icon overlay */}
  <TouchableOpacity onPress={() => navigation.navigate('Edit Profile')} style={styles.editIconContainer}>
    <Icon name="edit" size={16} color="#000" />
          ...
          <View style={styles.completionBadge}>
            {personalData && personalData.match_parsentence !== undefined ? (
              ...
              {/* <Text>{JSON.stringify(personalData, null, 2)}</Text> */}
              </>
            ) : (
...
const styles = StyleSheet.create({
 ...
 progressRing: {
  ...
  userName: {
   ...
   editIconContainer: {
    position: 'absolute',
    bottom: 0, // show near bottom edge
    right: 0,  // align to right edge
    backgroundColor: '#FFCA30',
    borderRadius: 20,
    padding: 6,
    // borderWidth: 2,
    borderColor: 'white', // creates clean border effect
    justifyContent: 'center',
    alignItems: 'center',
    elevation: 2, // adds slight shadow for better visibility
  },
  locationContainer: {
    flexDirection: 'row',
    alignItems: 'center',
